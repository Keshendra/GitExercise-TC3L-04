...
enemy2 = pygame.image.load("air.png").convert_alpha()
enemy2_img = pygame.transform.scale(enemy2, (PLAYER_WIDTH, PLAYER_HEIGHT))
...
def draw(hero, enemy2, hero_health_rect, enemy2_health_rect, spells):
    
    WINDOW.blit(hero_img, (hero.x, hero.y))
    WINDOW.blit(enemy2_img, (enemy2.x, enemy2.y))
    ...
    pygame.draw.rect(WINDOW, "red", enemy2_health_rect)
...
def main():
    ...
    enemy2 = pygame.Rect(WIDTH - PLAYER_WIDTH, HEIGHT - PLAYER_HEIGHT, PLAYER_WIDTH, PLAYER_HEIGHT)
    hero_health = PLAYER_HEALTH
    enemy2_health = PLAYER_HEALTH
    hero_health_rect = pygame.Rect(10, 10, hero_health * 3, 40)
    enemy2_health_rect = pygame.Rect(WIDTH - 300 - 10, 10, 300, 40)
    ...
    while run:
        ...
        if current_time - last_air_spell_time >= 3000:
            air_spell_rect = pygame.Rect(enemy2.x, enemy2.y + enemy2.height // 2 - 50, 100, 100)
        ...
        for event in pygame.event.get():
            ...
            for spell in spells[:]:
                if spell["type"] == "lightning":
                    ...
                    if spell["type"] == "lightning" and spell["rect"].colliderect(enemy2):
                        enemy2_health -= 10
                        enemy2_health_rect.width = enemy2_health * 3
                        spells.remove(spell)

                elif spell["type"] == "air":
                    spell["rect"].x -= SPELL_VEL
                    if spell["type"] == "air" and spell["rect"].colliderect(hero):
                        hero_health -= 10
        ...
        draw_IMG()    
        draw(hero, enemy2, hero_health_rect, enemy2_health_rect, spells)
